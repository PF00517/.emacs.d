...
(defvar pf-gc-cons-threshold 16777216) 
(setq gc-cons-threshold most-positive-fixnum 
      gc-cons-percentage 0.6 
      garbage-collection-messages t) 
(define-hook emacs-startup-hook () 
  (setq gc-cons-threshold pf-gc-cons-threshold 
        gc-cons-percentage 0.1)) 
(define-hook minibuffer-setup-hook () 
  (setq gc-cons-threshold most-positive-fixnum)) 
(define-hook minibuffer-exit-hook () 
  (run-at-time 1 nil (lambda () (setq gc-cons-threshold pf-gc-cons-threshold)))) 

;; Avoid special file handling during startup 
(defvar pf--file-name-handler-alist file-name-handler-alist) 
(setq file-name-handler-alist nil) 
(define-hook emacs-startup-hook () 
  (setq file-name-handler-alist pf--file-name-handler-alist)) 

(provide 'pf-faststart)